...
caches), forgets the reconciliation, and then opens the editor as usual. Remote-SSH
...
storage, a mount you declared, or the environment itself — and the environment keeps
running: nothing else in the guest is touched, `hooks.create` does not run again, and
windows already attached lose their server and reconnect. It waits for no running
reconciliation: one in progress has to finish first. `vk dev editor reset [--editor BIN]`
does the same without opening an editor; the environment must be up.
