...
/// Toolchain user agent: no individual `Tool` is being replaced, so no tool name and
...
/// The resolved release version, requested artifacts it publishes, and missing names.
///
...
async fn artifacts_at(
    ...
    for name in names {
        // Both absent means the artifact is missing. An incomplete pair is a broken
        // release; `assets_of` reports the missing half.
...
/// How many bytes a download may write before it is refused; the ceiling used when no
...
/// bounds and on ASCII boundaries. Public so `vk toolchain` and downloads use the same
/// digest validation.
...
mod tests {
    ...
    /// `OUT_DIR`, since this crate has no build script to set it.
    struct Scratch {
        dir: PathBuf,
        exe: PathBuf,
    }
