//! `vk toolchain`: a project's pinned virtkit release and its locally cached artifacts.
//!
//! `[requires] min-version` sets the oldest compatible `vk`. `.virtkit/toolchain.lock`,
//! tracked beside the config, pins the exact release for laptops, Docker builds and CI.
//! It records digests and download URLs per artifact and platform. Verification uses
//! the locked digest, so mirrors must supply the same bytes as GitHub.
//!
//! Installation fills a versioned directory in the user's cache without touching `vk`
//! on PATH: an older project pin must not silently downgrade the user's general-purpose
//! tool. `export` supplies cached paths and digests to shells, Dockerfile build args and
//! CI image builders without requiring them to parse the lock.
...
/// A tracked lock is small — a version and a handful of artifacts. Bound the read so a
/// bloated or hostile `.virtkit/toolchain.lock` cannot OOM the boot path that consults it on
/// every `vk dev up`; the network reads this feature makes are bounded the same way.
const MAX_LOCK: u64 = 256 * 1024;
const LOCK_READ_CAP: u64 = MAX_LOCK + 1;

fn read_lock(path: &Path) -> Result<Lock> {
    let mut text = String::new();
    std::fs::File::open(path)
        .and_then(|f| f.take(LOCK_READ_CAP).read_to_string(&mut text))
        .with_context(|| format!("reading {}", path.display()))?;
    if text.len() as u64 > MAX_LOCK {
        bail!(
            "{} is larger than a toolchain lock should be",
            path.display()
        );
    }
    let mut lock: Lock =
    ...
    for (name, per_platform) in &mut lock.artifacts {
        ...
        for (platform, entry) in per_platform.iter_mut() {
            ...
            // Validate and lowercase digests for comparison with `digest_of`'s hex output.
            // A malformed digest would parse but never match a correctly installed file.
            if vk_selfupdate::parse_sha256(&entry.sha256).is_none() {
                bail!(
                    "{} gives {name} on {platform} a sha256 of {:?}, which is not a digest",
                    path.display(),
                    entry.sha256
                );
            }
...
/// Write `lock` to `path` atomically, so a crash cannot leave a half-written lock.
...
/// Check that the cached file exists and matches the locked digest. Rehash files in this
/// ordinary user directory so interrupted downloads or edits cannot reach a build as
/// locked artifacts. Treat unreadable files as not installed, allowing replacement.
...
/// install that installs nothing must fail, not exit 0.
fn plan_install(
    lock: &Lock,
    dir: &Path,
    platform: &str,
...
/// `VIRTKIT_VK_AGENT`, with `_SHA256` beside it for the digest. Every character that is not a
/// shell identifier — `-`, and a `.` a `--artifact` name may carry — becomes `_`, so the
/// result is always a name `eval` accepts.
...
fn export_vars(
    ...
    for (name, per_platform) in &lock.artifacts {
        ...
        let path = dir.join(name);
        let var = var_of(name);
        let value = match installed(&path, &entry.sha256) {
            // The export is a shell word or a JSON string either way, so a cache path that
            // is not UTF-8 is refused rather than silently mangled into one.
            true => path
                .to_str()
                .with_context(|| format!("the cache path {} is not UTF-8", path.display()))?
                .to_string(),
            false => String::new(),
        };
...
fn render_export(vars: &[(String, String)], format: Format) -> Result<String> {
    let mut out = String::new();
    match format {
        // Assignments can be evaluated or sourced; callers export values to children
        // themselves.
        Format::Shell => {
        ...
        Format::Json => {
            let map: serde_json::Map<String, serde_json::Value> = vars
                ...
                .collect();
            let text = serde_json::to_string_pretty(&map).context("serializing the export")?;
            out.push_str(&text);
            out.push('\n');
        }
    }
...
mod tests {
    ...
    fn a_lock_that_could_escape_the_cache_is_refused() {
        ...
        let entry = format!("sha256 = \"{SUM_VK}\"\nurls = [\"https://example/x\"]\n");
    ...
    fn export_names_and_quotes_every_value() {
        ...
        let lock = sample();
        let missing = export_vars(&lock, dir, "linux-x86_64", &|_, _| false).unwrap();
        assert_eq!(
            missing,
            vec![
        ...
        let installed = export_vars(&lock, dir, "linux-x86_64", &|_, _| true).unwrap();
        ...
        let stale = export_vars(&lock, dir, "linux-x86_64", &|_, sha| sha == SUM_VK).unwrap();
        ...
        assert_eq!(
            export_vars(&lock, dir, "macos-aarch64", &|_, _| true)
                .unwrap()
                .len(),
            1
        );

        let shell = render_export(&installed, Format::Shell).unwrap();
        ...
        assert_eq!(
            render_export(&[("V".into(), "a'b".into())], Format::Shell).unwrap(),
            "V='a'\\''b'\n"
        );
        let json = render_export(&installed, Format::Json).unwrap();
        assert!(
    ...
    // A mirror that fails, and one that serves the wrong bytes, are both skipped; the next
    // URL that delivers the locked bytes wins — the reason a lock carries several. The fake
    // follows the real fetch's contract: a URL that errors leaves nothing at `dest`, so the
    // wrong bytes are never what ends up cached.
    #[tokio::test]
    async fn install_fails_over_to_a_working_mirror() {
        use std::sync::Mutex;

        struct Failover {
            tried: Mutex<Vec<String>>,
        }
        impl Fetch for Failover {
            async fn fetch(&self, url: &str, sha: &str, dest: &Path, _mode: u32) -> Result<()> {
                self.tried.lock().unwrap().push(url.to_string());
                if url.contains("unreachable") {
                    // A network failure: the real fetch writes nothing.
                    bail!("connection refused");
                }
                if url.contains("wrong-bytes") {
                    // Bytes that do not hash to the lock: the real fetch's digest gate
                    // rejects them and leaves `dest` untouched.
                    bail!("{url} does not match the locked digest");
                }
                let bytes = match sha {
                    SUM_VK => BYTES_VK,
                    other => bail!("nothing published under {other}"),
        ...
        per_platform.insert(
            ...
            Entry {
                sha256: SUM_VK.to_string(),
                urls: vec![
                    "https://unreachable.example/vk".to_string(),
                    "https://wrong-bytes.example/vk".to_string(),
                    "https://good.example/vk".to_string(),
                ],
            },
        );
        let mut artifacts = BTreeMap::new();
        artifacts.insert("vk".to_string(), per_platform);
        let lock = Lock {
        ...
        run_install(&plan, &dir, false, &how).await.unwrap();

        // The good mirror's bytes are what landed, and only after the two bad URLs were
        // tried in order.
        assert_eq!(std::fs::read_to_string(dir.join("vk")).unwrap(), BYTES_VK);
        let tried = how.tried.lock().unwrap().clone();
        assert_eq!(
            tried,
            vec![
                "https://unreachable.example/vk".to_string(),
        ...
        // Installed, and seen as cached on a re-plan.
        let plan = plan_install(&lock, &dir, "linux-x86_64", &[]).unwrap();
        assert!(plan.iter().all(|p| p.step == Step::Cached), "{plan:?}");
    }

    // Fail-fast: a digest that is not 64 hex is refused when the lock is read, before any
    // path or install is built from it.
    #[test]
    fn read_lock_rejects_a_malformed_sha256() {
        let dir = tmpdir("bad-sha");
        let path = dir.join("toolchain.lock");
        let write = |sha: &str| {
            std::fs::write(
                &path,
        ...
        write(&"z".repeat(64)); // 64 characters, but not hex
        assert!(read_lock(&path).is_err(), "non-hex digest");
        write(&"a".repeat(40)); // hex, but too short for a sha256
        assert!(read_lock(&path).is_err(), "wrong-length digest");
        write(SUM_VK); // a real digest still reads
        assert!(read_lock(&path).is_ok());
    }

    // Uppercase digests read back lowercase, keeping installed artifacts cached instead
    // of re-fetching them and exporting real paths instead of empty mismatch paths.
    #[tokio::test]
    async fn an_uppercase_digest_normalises_and_stays_installed() {
        struct Fake;
        impl Fetch for Fake {
            async fn fetch(&self, _url: &str, sha: &str, dest: &Path, _mode: u32) -> Result<()> {
                assert_eq!(sha, SUM_VK, "the digest reaches fetch already lower-cased");
                std::fs::write(dest, BYTES_VK)?;
                Ok(())
            }
        }

        let dir = tmpdir("uppercase");
        let path = dir.join("toolchain.lock");
        std::fs::write(
            &path,
            format!(
                "version = \"0.60.0\"\n[artifacts.vk.linux-x86_64]\nsha256 = \"{}\"\n\
                 urls = [\"https://good.example/vk\"]\n",
                SUM_VK.to_uppercase()
            ),
        )
        .unwrap();
        ...
        let sha = lock.artifacts["vk"]["linux-x86_64"].sha256.clone();
        assert_eq!(sha, SUM_VK, "read_lock lower-cased the digest");

        let cache = dir.join("cache/0.60.0");
        std::fs::create_dir_all(&cache).unwrap();
        let plan = plan_install(&lock, &cache, "linux-x86_64", &[]).unwrap();
        run_install(&plan, &cache, false, &Fake).await.unwrap();

        // is_installed and a re-plan both see the artifact, and export emits its path.
        assert!(is_installed(&cache.join("vk"), &sha));
        let plan = plan_install(&lock, &cache, "linux-x86_64", &[]).unwrap();
        assert!(plan.iter().all(|p| p.step == Step::Cached), "{plan:?}");
        let vars = export_vars(&lock, &cache, "linux-x86_64", &is_installed).unwrap();
        let vk = vars.iter().find(|(k, _)| k == "VIRTKIT_VK").unwrap();
        assert_eq!(vk.1, cache.join("vk").to_str().unwrap());
    }

    // The `vk dev up` note: only a vk older than the pinned release is worth a word, and a
    // malformed lock produces no note rather than breaking the boot path.
    #[test]
    fn older_than_lock_notes_only_an_older_vk() {
        let ws = tmpdir("older");
        let lockdir = ws.join(".virtkit");
        std::fs::create_dir_all(&lockdir).unwrap();
        let path = lockdir.join("toolchain.lock");
        let write = |ver: &str| {
            std::fs::write(
                &path,
                format!(
                    "version = \"{ver}\"\n[artifacts.vk.linux-x86_64]\nsha256 = \"{SUM_VK}\"\n\
                     urls = [\"https://example/vk\"]\n"
                ),
            )
            .unwrap();
        };

        // A release newer than this build is the one case that speaks.
        write("99.0.0");
        assert_eq!(older_than_lock(&ws).as_deref(), Some("99.0.0"));
        // This build's own version, and an older release, stay quiet.
        write(env!("CARGO_PKG_VERSION"));
        assert_eq!(older_than_lock(&ws), None);
        write("0.1.0");
        assert_eq!(older_than_lock(&ws), None);
        // A malformed lock does not panic the note path; it simply produces no note.
