...
/// Run one hook. A [`HookPlan::Group`] attempts every member sequentially in name order:
/// members share a guest and may depend on each other. Any required failure fails the
/// group; best-effort failures are reported and execution continues. Add `env` to the
/// guest's `exec-env` or the host environment.
...
async fn run_in_guest(
    ...
    // Only a clean exit 0 is success. A `None` code is a signal kill or a dropped exec
    // channel — a hook that did not run to completion, which `create` must not stamp as done
    // (its stamp would suppress the initializer for good) and a `start` must not pass off as
    // a booted-and-ready environment.
    match result.code {
        Some(0) => Ok(()),
        Some(code) => bail!("{name} exited {code}"),
        None => bail!("{name} did not complete (signal {:?})", result.signal),
    }
}

/// The hooks that run once the environment is up after a boot: `create` for a generation
/// that has not had it, then `start`. A *required* hook's failure stops the ones after it and
/// leaves no stamp saying it succeeded; a `required = false` `create` that fails is reported
/// and skipped over, and still stamped — `create` runs once per generation, not once per
/// boot. A `None` generation — nothing filed for the booted VM to read it off — runs `create`
/// and stamps nothing, so the next boot runs it again.
///
