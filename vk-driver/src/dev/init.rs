//! `vk dev init`: create `.virtkit/config.toml` from an existing `devcontainer.json`,
//! compose file or Dockerfile, or from a stock image.
//!
//! Read the source and write a draft with a report of translated settings, decisions and
//! omissions. Import executes no hooks, downloads no tools and boots no VMs. Missing
//! essential choices, such as the compose service or Dockerfile stage, leave a draft
//! for the reader to finish and a failing exit status so scripts cannot mistake it for
//! a complete config.
...
/// Run `init` for `cwd`, or for `workspace` when `--workspace` names it. An explicit
/// workspace wins verbatim, like every other `vk dev` subcommand; without one, anchor to
/// the checkout root so a call from a subdirectory writes or validates the project's own
/// config rather than a second one inside it.
pub fn run(cwd: &Path, workspace: Option<&Path>, opts: &Opts) -> Result<Outcome> {
    let (workspace, existing) = match workspace {
        Some(w) => {
            let w = crate::dev::config::absolute(w)?;
            let existing = crate::dev::config::config_in(&w)?;
            (w, existing)
        }
        None => {
            let existing = crate::dev::config::discover_here(cwd)?;
            let workspace = match &existing {
                Some(files) => files.workspace.clone(),
                None => crate::dev::config::worktree_root(cwd).unwrap_or_else(|| cwd.to_path_buf()),
            };
            (workspace, existing)
        }
    };
    ...
    // An existing config is validated, not replaced: it, not a fresh translation, is the
    // project's current description.
    ...
    let (draft, read) = match source {
        Source::Devcontainer => {
            ...
            let text = crate::dev::config::read_regular(&path)?
                .with_context(|| format!("{} is gone", path.display()))?;
    ...
    // Reload through the usual config loader to report errors before the first `vk dev shell`.
    match crate::dev::config::discover(&workspace, Some(&workspace), None)
...
fn from_compose(path: &Path, workspace: &Path) -> Result<Draft> {
    ...
    let rel = rel
        .to_str()
        .with_context(|| format!("{}: path is not UTF-8", rel.display()))?
        .to_string();
...
fn from_dockerfile(path: &Path) -> Result<Draft> {
    ...
    let src = crate::dev::config::read_regular(path)?
        .with_context(|| format!("{} is gone", path.display()))?;
    let stages = crate::build::dockerfile_stages(&src)
        .with_context(|| format!("parsing {}", path.display()))?;
    if stages.len() > 1 {
        // Written empty, not omitted: `--target ""` builds nothing, so the draft names the
        // key and fails validation until the reader picks a stage.
...
mod tests {
    ...
    fn nothing_to_read_from_writes_the_template_and_an_existing_config_is_validated() {
        ...
        let out = run(&f.0, None, &opts()).unwrap();
        ...
        let out = run(&sub, None, &opts()).unwrap();
        assert!(
        ...
        assert!(run(&f.0, None, &o).is_err());
        o.force = true;
        let out = run(&f.0, None, &o).unwrap();
    ...
    #[test]
    fn an_explicit_workspace_is_honored_over_the_checkout_root() {
        let f = project("workspace");
        let sub = f.0.join("service");
        std::fs::create_dir_all(&sub).unwrap();
        // With `--workspace`, the config is written under the named directory, not anchored
        // up to the checkout root the way a bare cwd call would be.
        let out = run(&f.0, Some(&sub), &opts()).unwrap();
        assert!(out.ok, "{}", out.report);
        assert!(
            sub.join(".virtkit").exists(),
            "wrote under the named workspace"
        );
        assert!(!f.0.join(".virtkit").exists(), "not at the checkout root");
        assert_eq!(
            std::fs::read_to_string(sub.join(crate::dev::config::CONFIG_FILE)).unwrap(),
            crate::dev::config::TEMPLATE
        );
        // A parent's config is never substituted for the named workspace's own: a broken one
        // at the root does not become what this call reports.
        ...
        assert!(
            out.ok && out.report.contains("config.toml: ok"),
            "validated the workspace's own config, not the parent's: {}",
            out.report
        );
    }

    /// A devcontainer with separate Docker and VM compose files and services;
    /// `customizations.virtkit` selects the VM's.
  ...
  "customizations": {{
    ...
    fn a_named_source_that_is_not_there_says_so_and_an_existing_config_is_read() {
        ...
        for (from, expect) in [
            ...
            let mut o = opts();
            o.from = Some(from);
            let msg = format!("{:#}", run(&f.0, None, &o).unwrap_err());
        ...
        .unwrap();
        let msg = format!("{:#}", run(&f.0, None, &opts()).unwrap_err());
    ...
    fn an_image_source_takes_an_explicit_reference() {
        ...
        assert!(run(&f.0, None, &o).is_err(), "needs --image");
        ...
        o.force = true;
        run(&f.0, None, &o).unwrap();
        assert!(config_of(&f).contains("image = "), "{}", config_of(&f));
    }

    #[test]
    fn image_only_goes_with_from_image() {
        let f = project("image-conflict");
        let mut o = opts();
        o.image = Some("alpine:3".into());
        o.from = Some(Source::Dockerfile);
        let msg = format!("{:#}", run(&f.0, None, &o).unwrap_err());
        assert!(msg.contains("--image only goes with --from image"), "{msg}");
    }

    #[test]
    fn a_compose_file_with_no_services_is_reported() {
        let f = project("no-services");
        std::fs::write(f.0.join("compose.yaml"), "version: \"3\"\n").unwrap();
        let out = run(&f.0, None, &opts()).unwrap();
        assert!(!out.ok, "{}", out.report);
        assert!(
            out.report.contains("declares no services"),
            "{}",
            out.report
        );
    }
