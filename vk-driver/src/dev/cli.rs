...
enum DevAction {
    ...
    Shell {
        ...
        /// workspace directory: the shell runs as the service's default user, in its default
        /// directory.
        #[arg(long, value_name = "NAME")]
        service: Option<String>,
    },
...
async fn dev_action(
    ...
    match action {
        ...
        DevAction::Shell {
            ...
            match dev::exec_in_service(&plan, &service, &argv, None, true, None).await {
                Ok(result) => exec::exit(result),
                Err(e) => fail(&e, 1),
            }
        }
        DevAction::Shell { service: None } => {
            match dev_up(&plan, host_cfg, over, false, true, false).await {
                Ready::Done(code) => code,
                Ready::Act => {
                    let argv: Vec<String> =
                        dev::LOGIN_SHELL.iter().map(|s| s.to_string()).collect();
        ...
        DevAction::Code {
            ...
            Ready::Act => {
                ...
                // Before the reconciliation starts: it would otherwise apply to a server
                // about to be emptied and leave a stamp for it. And only once the editor can
                // launch — no point emptying the server if the open would fail.
                if reset_server {
