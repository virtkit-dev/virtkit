...
/// The parent of the environments' state directories, used for the shared lock and
/// the scan for other environments' allocations.
...
///
/// An unreadable file is an error: treating it as absent would allocate a new block
/// and change every published URL.
...
/// For when a publish finds its address taken after all. Returns whether the octet was
/// actually dropped, so a caller only reports it as forgotten when it was.
///
...
pub(crate) fn forget(plan: &Plan, service: Option<&str>) -> bool {
...
/// Serialize allocations so two environments cannot reserve the same block between
/// the bind probe and publication.
fn global_lock(plan: &Plan) -> Result<std::fs::File> {
    ...
    // Private, and not through a symlink planted at the name, as `publish::lock_file` opens
    // its own locks: this file sits in a directory the state dirs share.
...
pub(crate) fn allocate(plan: &Plan, service: Option<&str>, live: bool) -> Result<Ipv4Addr> {
        ...
        && let Some(addr) = a.address(service)
    {
        ...
        // Another listener took a port on our address. Active relays pin the block,
        // so the caller must resolve the conflict.
        if live {
            bail!(
...
/// Refuse a configured host address that is not loopback. A published endpoint binds a guest
/// port on the host, so a tracked config a clone brought must not reach it onto the LAN;
/// `auto` and `127.0.0.0/8`/`::1` are the only addresses that stay on this machine.
pub(crate) fn require_bindable(name: &str, address: &str) -> Result<()> {
    let ip: std::net::IpAddr = address
        .parse()
        .with_context(|| format!("[endpoint {name}] address {address:?}"))?;
    if !ip.is_loopback() {
        bail!(
            "[endpoint {name}] address {address} is not loopback: it would expose the \
             guest port on every interface"
        );
    }
    Ok(())
}

/// `<address>:<port>`, bracketing an IPv6 literal so the result parses as a socket address
/// and as a URL authority — a configured `::1` renders as `[::1]:<port>`, not the
/// unparseable `::1:<port>`. Auto addresses are IPv4, so this only ever changes a
/// configured loopback address a caller wrote as `::1`.
pub(crate) fn authority(address: &str, port: u16) -> String {
...
pub(crate) fn listen_on(ep: &EndpointPlan, address: &str) -> String {
    format!("tcp://{}", authority(address, ep.host_port))
}
...
pub(crate) fn url_on(ep: &EndpointPlan, address: &str) -> Option<String> {
    ...
    Some(format!(
        "{scheme}://{}{}",
        authority(address, ep.host_port),
...
pub(crate) fn render(views: &[View]) -> String {
    ...
    let rows: Vec<Vec<String>> = views
        ...
        .collect();
    crate::dev::table::render(&["NAME", "SERVICE", "ADDRESS", "TO", "STATE"], &rows)
}

#[cfg(test)]
mod tests {
    #[test]
    fn only_a_loopback_configured_address_is_publishable() {
        super::require_bindable("x", "127.0.0.1").unwrap();
        super::require_bindable("x", "::1").unwrap();
        for exposed in ["0.0.0.0", "::", "192.168.1.5", "10.0.0.1"] {
            let msg = format!("{:#}", super::require_bindable("x", exposed).unwrap_err());
            assert!(msg.contains("not loopback"), "{exposed}: {msg}");
        }
        assert!(super::require_bindable("x", "not-an-ip").is_err());
    }
    ...
    /// Isolate the test's state directory in its own base. The allocator locks and
    /// scans that base, leaving the host's environments untouched.
    ...
    fn forget_drops_one_services_octet_and_leaves_the_block() {
        ...
        assert!(forget(&plan, Some("a")), "the octet was dropped");
    ...
    fn an_unreadable_allocation_is_an_error_and_not_a_fresh_start() {
        ...
        // A truncated file, as a crash mid-write would leave.
    ...
    #[test]
    fn a_configured_ipv6_loopback_produces_a_bracketed_parseable_authority() {
        // `require_bindable` blesses `::1`, so the listen address and URL it turns into must
        // be spellings that actually parse — which for an IPv6 literal means brackets.
        require_bindable("x", "::1").unwrap();
        let mut ep = ep("web", None, 8443, Some("https"));
        ep.address = "::1".into();
        let listen = listen_on(&ep, "::1");
        assert_eq!(listen, "tcp://[::1]:8443");
        assert!(
            listen.parse::<vk_core::addr::SocketAddr>().is_ok(),
            "{listen}"
        );
        assert_eq!(url_on(&ep, "::1").as_deref(), Some("https://[::1]:8443/ui"));
        // An IPv4 loopback is left exactly as it was.
        assert_eq!(listen_on(&ep, "127.0.0.1"), "tcp://127.0.0.1:8443");
    }
