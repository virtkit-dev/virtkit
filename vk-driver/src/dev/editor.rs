...
//! as a detached host process and joined by later attachments. The editor launches
//! immediately: VM readiness never waits for the server, and the server never blocks this.
//!
...
impl Channel {
    ...
    /// The host extension that makes Remote-SSH work for this channel. Finding the editor
    /// does not prove it can attach; the presence of one of these does.
...
pub fn select(name: Option<&str>) -> Result<Editor> {
    ...
    // Use the resolved binary's name: `--editor /opt/vscode/bin/code-insiders` selects
    // insiders and `.vscode-server-insiders`. The detached reconciliation receives the
    // same path and must select the same channel.
    let channel = Channel::of(&base_name(&binary));
    let output = std::process::Command::new(&binary)
        .arg("--version")
        .output()
        .with_context(|| format!("running {} --version", binary.display()))?;
    let version_out = std::str::from_utf8(&output.stdout)
        .with_context(|| format!("{} --version: non-UTF-8 output", binary.display()))?;
    let (version, commit) =
        parse_version(version_out).with_context(|| format!("{} --version", binary.display()))?;
...
/// command, and the server they were applied to. A change to any means it runs again.
///
...
/// it; a shell opened in the editor's terminal does not, and stays — told to leave, given a
/// moment to, and killed if they take longer, before their files go. The contents go, not
/// the directory itself, which may be a mount point. What is left behind is reported, and a
/// non-zero exit says so.
...
/// channel goes, not this commit's alone: they all describe the directory just emptied.
...
pub async fn reconcile(plan: &Plan, editor: &Editor) -> Result<Outcome> {
    ...
    // A stamp records a prior run; re-check the extensions before trusting it.
    ...
    vk_fs::write_atomic(&stamp, want.as_bytes(), 0o600)
        .with_context(|| format!("publishing {}", stamp.display()))?;
    println!("virtkit: editor: reconciled ({})", &want[..12]);
    Ok(Outcome::Done)
}

/// The extensions all present, in one pass. Invoked as `sh -c <script> <cli> <ext…>`, so
/// `$0` is the server CLI and `$1..` the extension ids — `for id` iterates exactly those.
/// Any one missing exits non-zero.
const ALL_PRESENT: &str = "list=$(\"$0\" --list-extensions 2>/dev/null) || exit 1; \
                           for id do \
                           printf '%s\\n' \"$list\" | grep -qixF -- \"$id\" || exit 1; \
                           done";

/// The one extension `$1` present. Invoked as `sh -c <script> <cli> <ext>`.
const ONE: &str = "\"$0\" --list-extensions 2>/dev/null | grep -qixF -- \"$1\"";

/// Which of `wanted` the server does not have.
///
/// Check all extensions with one `--list-extensions` call; probe each separately only if
/// that fails. `grep -F` matches literal ids: without it, `.` is a regex wildcard and can
/// make a near-match count as installed.
...
/// the first that answers is returned; only a server of this commit qualifies.
...
/// A `<channel>-<commit>` slug with the commit cut to the twelve characters used elsewhere;
/// the full hash only widens the output.
...
/// How much of a log `vk dev editor log` prints. A long wait for the server can produce
/// more log than anyone reads, and the tail is what shows the outcome.
...
pub fn latest_log(plan: &Plan) -> Result<String> {
    ...
    let path = entry.path();
    // Raw guest output, so it may not be UTF-8; render lossily rather than fail to print it.
    let bytes = std::fs::read(&path).with_context(|| format!("reading {}", path.display()))?;
    let mut out = format!("== {}\n", path.display());
    match bytes.len() > LOG_TAIL_BYTES {
        true => {
            let cut = bytes.len() - LOG_TAIL_BYTES;
            out.push_str(&format!("… {cut} earlier byte(s) not shown\n"));
            out.push_str(&String::from_utf8_lossy(&bytes[cut..]));
        }
        false => out.push_str(&String::from_utf8_lossy(&bytes)),
    }
    Ok(out)
}
...
mod tests {
    ...
    fn the_extension_probes_check_every_id_over_a_real_shell() {
        ...
        // against a stub `code` to guard that argument layout — a stray `shift` here once
        // dropped the first id, so a missing first extension read as "all present".
        let dir = std::env::temp_dir().join(format!("vk-editor-probe-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        let code = dir.join("code");
        // Lists ext2 and ext3, but not the first requested id, ext1.
        ...
        drop(f);
        let cli = code.to_str().unwrap();

        let all_present = |exts: &[&str]| -> bool {
            let mut args = vec![cli];
            args.extend_from_slice(exts);
        ...
        assert!(
            !all_present(&["ext1", "ext2", "ext3"]),
            "a missing first extension must be caught"
        );
        assert!(all_present(&["ext2", "ext3"]), "every id present");

        let one = |ext: &str| -> bool {
            std::process::Command::new("/bin/sh")
                .arg("-c")
                .arg(ONE)
                .args([cli, ext])
