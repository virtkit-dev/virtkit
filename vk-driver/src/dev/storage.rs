...
/// An item's intended lifetime, which determines its storage category.
...
/// Active owners. Removal requires a stopped owner; the environment owns items without
/// a service owner.
...
///
/// Propagate control-plane errors: treating an unanswered request as an empty list
/// would let a reset remove a live service's disk.
...
/// server; deeper than this is not a backing we created; the cap bounds the recursion.
...
pub async fn reset(plan: &Plan, name: &str, yes: bool) -> Result<String> {
    ...
    if up.owns(item.owner.as_deref()) {
        ...
        // Nothing is running, but the backing is durable — confirm before destroying it.
        if !crate::dev::on_terminal() {
            bail!("{name}: pass --yes to remove {backing}");
        }
        if !crate::dev::ask_on_terminal(&format!("remove {backing}?"))? {
            return Ok(format!("{name}: kept"));
        }
    }
...
fn remove_backing(backing: &Path) -> Result<()> {
    ...
    match st.st_mode & libc::S_IFMT {
        libc::S_IFDIR => {
            // `remove_dir_all` re-resolves `backing` rather than acting through `dir`, so it
            // repeats the lookup fstatat just did. The residual TOCTOU is acceptable here: a
            // single-user rootless tool, and `remove_dir_all` will not follow a symlinked leaf.
...
/// Preview which storage a rebuild keeps or recreates.
/// Return no storage section if none is declared or the inventory cannot be read.
...
mod tests {
    ...
    #[test]
    fn reset_without_yes_keeps_a_stopped_durable_backing() {
        let (t, plan) = fixture("confirm");
        let rt = tokio::runtime::Builder::new_current_thread()
            .build()
            .unwrap();
        let disk = t.0.join("repo/.virtkit/runner-var-wab.qcow2");
        std::fs::write(&disk, b"data").unwrap();
        // Nothing is running and the harness's stdin is not a tty, so the default (no --yes)
        // path refuses rather than destroying durable data unasked.
        let e = rt
            .block_on(reset(&plan, "runner:/var/wab", false))
            .unwrap_err();
        assert!(format!("{e:#}").contains("pass --yes"), "{e:#}");
        assert!(
            disk.is_file(),
            "the backing was removed without a confirmation"
        );
    }

    #[test]
    fn reset_refuses_an_environment_owned_backing_while_booting() {
        let (t, plan) = fixture("booting-env");
        let rt = tokio::runtime::Builder::new_current_thread()
            .build()
            .unwrap();
        // An `owner == None` item: the environment's own `${state}` managed directory.
        let backing = t.0.join("state/vscode-server");
        std::fs::create_dir_all(&backing).unwrap();
        // A boot in flight holds the state-dir lock with no VM registered on it yet.
        let held =
            crate::dev::list::try_lock_state_dir(&plan.state_dir).expect("free to begin with");
        let e = rt
            .block_on(reset(&plan, "${state}/vscode-server", true))
            .unwrap_err();
        assert!(format!("{e:#}").contains("being booted"), "{e:#}");
        assert!(
            backing.is_dir(),
            "the managed dir was removed under the boot"
        );
        drop(held);
    }
