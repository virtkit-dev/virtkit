...
//!
//! When `[dev.host] git-gui = true`, the guest's host commands reach `vk host-policy git-gui`
//! over the exec channel, and this is where each one is decided: on argv alone, handed back a
//! command whose environment is rebuilt from nothing, so nothing the client sent selects what
//! runs.
//!
...
/// The `-`-prefixed options a caller may pass. Only options are allowlisted; the scan lets a
/// non-`-` positional through, since gitk and git gui take those as revisions or paths — data
/// naming what to show, which executes no code — do not extend this policy to a command that
/// reads a positional otherwise. Options that load code (`gitk --argscmd` runs its value in a shell)
/// are refused by not being here. `--` is included because the scan checks every `-`-word
/// wherever it sits: see [`git_gui`].
...
/// The policy's decision for one request.
...
/// Only `-`-prefixed words are checked against the allowlist; a positional is forwarded to
/// gitk/git gui unchanged, safe only because those take it as a revision or path — data that
/// names what to show and runs no code. `--` does not end the scan, since it is gitk's own option
/// parser — not this one — that would have to stop there for that to be safe. A pathspec
/// that starts with `-` is spelled `./-x`, which is not an option to anything.
///
...
pub fn git_gui(
    ...
    match std::fs::canonicalize(cwd) {
        ...
        // Distinguish a missing or unresolvable directory from one outside the workspace.
        Err(e) => {
            return refuse(format!(
...
///
/// Both become command environment values. Reject invalid UTF-8 rather than replacing
/// characters, which could make `HOME` point to a different directory.
...
mod tests {
    ...
    #[test]
    fn repo_redirection_and_global_options_are_refused() {
        // A global option before the subcommand does not read as `git gui`.
        assert!(refused(&["git", "-c", "core.sshCommand=x", "gui"]).contains("not 'git -c'"));
        // The canonical repo-redirection flags are not on the option allowlist.
        assert!(refused(&["gitk", "--git-dir=/evil"]).contains("--git-dir"));
        assert!(refused(&["gitk", "--work-tree=/evil"]).contains("--work-tree"));
        // Only the `=` form of an allowed option is accepted; the split form is refused.
        assert!(refused(&["gitk", "--max-count", "5"]).contains("--max-count"));
        // A bare `-` is not an allowlisted option.
        assert!(refused(&["gitk", "-"]).contains("-"));
    }
    ...
    fn git_guis_own_subcommands_are_an_allowlist_too() {
        ...
        // An allowed subcommand execs `git` with the subcommand carried through as argv.
        assert_eq!(
            ran(&["git", "gui", "blame"]),
            ("git".into(), vec!["gui".into(), "blame".into()])
        );
    }

    #[test]
    fn an_empty_home_in_the_passwd_entry_is_refused() {
        // A passwd entry with no home directory names no configuration tree to run under.
        let argv = vec!["gitk".to_string()];
        let d = git_gui(&workspace(), &workspace(), &argv, &[], ("dev", "")).unwrap();
        assert!(
            matches!(&d, Decision::Refused(why) if why.contains("home directory")),
            "{d:?}"
        );
    }
