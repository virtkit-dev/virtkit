...
/// The smallest file `Dax::Inode` maps by default. The 1M floor sits well above source files,
/// which never repay a 2 MiB mapping (measured: 30k files of 2K), and below archives and disk
/// images that benefit from the shared host page cache. The crossover was not measured;
/// `<window>:inode=<min>` adjusts the floor.
pub const DAX_INODE_MIN_DEFAULT: u64 = 1 << 20;

/// One host page. A mapping is page-granular, so a floor below this cannot select anything
/// the mapping distinguishes; `inode=<min>` rejects anything smaller. Files under the floor
/// read through the guest page cache.
const DAX_INODE_MIN_FLOOR: u64 = 4096;

/// A share's DAX window and optional per-inode size floor.
...
impl Dax {
    /// This policy's window and size floor, or `None` when off.
        ...
        match mode {
            ...
            Some(m) => {
                let min = m
                    .strip_prefix("inode=")
                    .and_then(parse_size)
                    // Below one page the floor selects nothing a page-granular mapping can
                    // distinguish; above the window nothing would ever map.
