//! `vk update`: replace the running `vk` with a published GitHub release build.
//!
//! A release ships the host tool as one static asset (`vk`) next to the
//! `sha256sum` sidecar CI generates for it, and `vk` carries the guest kernel and
//! agent inside it — so an update is a single file: download it, check it against
//! the published digest, and rename it over the running binary. The rename is
//! atomic and leaves the live process untouched (it keeps the old inode; only the
//! directory entry moves), so an update never interrupts VMs already running.
//!
//! Nothing is written before the user confirms the version they are moving to, and
//! the replacement only happens once the download hashes to the digest published
//! beside it and the new binary reports its own version when run. Both gates are
//! integrity checks against one release rather than proof of who published it: they
//! catch a corrupted or truncated transfer and a build that cannot run here, which is
//! why the tag a release is looked up by has to be trusted in its own right — see
//! [`release_tag`]. `--check` stops after resolving the release, making it a read-only
//! query for what is available.
...
/// The release asset carrying the host binary; its digest rides in `<ASSET>.sha256`.
const ASSET: &str = "vk";
...
/// The largest `vk` a release can plausibly ship — the binary is tens of megabytes. The
/// download's own length check is relative to what the release announced, so without a
/// ceiling on that number a release claiming half a terabyte would be honoured.
...
/// A release's `vk` asset, resolved and ready to fetch.
#[derive(Debug)]
struct Target {
    tag: String,
    url: String,
...
enum Step {
    ...
    /// order at all, which [`smoke_test`] then refuses because the build cannot report
    /// the tag's own name as its version.
    Other,
}
...
/// What both subcommands resolve before deciding anything: the binary they would
...
async fn plan(api: &str, tag: Option<&str>) -> Result<Plan> {
    let exe = std::env::current_exe().context("locating the running vk binary")?;
    let client = http_client()?;
    let target = resolve(&client, api, tag).await?;
    let step = step(env!("CARGO_PKG_VERSION"), &target.tag);
    Ok(Plan {
        exe,
        client,
        target,
...
/// `vk update`: move this `vk` to `tag`'s release build, or to the latest release
/// when no tag is given. Prompts before replacing the binary unless `assume_yes`.
pub async fn run(tag: Option<&str>, assume_yes: bool) -> Result<()> {
    let plan = plan(API, tag).await?;
    let current = env!("CARGO_PKG_VERSION");
    if plan.step == Step::Same {
        println!("vk is already at {current} ({})", plan.exe.display());
        return Ok(());
    }
    ...
    println!("vk updated to {}", plan.target.tag);
    Ok(())
}

/// `vk update --check`: report how the release `tag` names — or the latest one when no
/// tag is given — compares to this binary, without downloading it. Returns true only
/// for a strictly newer release, which the caller turns into the exit code a script
/// can branch on.
pub async fn check(tag: Option<&str>) -> Result<bool> {
    ...
    println!("vk {current} ({})", plan.exe.display());
    match plan.step {
        Step::Same => {
        ...
        Step::Newer => {
            ...
            let hint = match tag {
                Some(_) => format!("vk update {found}"),
                None => "vk update".to_string(),
            };
            println!(
...
fn http_client() -> Result<reqwest::Client> {
    reqwest::Client::builder()
        .user_agent(concat!("vk/", env!("CARGO_PKG_VERSION")))
...
/// Resolve a release — `tag`'s, or the latest published one — to its `vk` asset.
async fn resolve(client: &reqwest::Client, api: &str, tag: Option<&str>) -> Result<Target> {
    ...
    let asset = pick(&release.assets, ASSET)?;
    let digest = pick(&release.assets, &format!("{ASSET}.sha256"))?;
    if asset.size > MAX_ASSET {
        bail!(
            "the release's {ASSET} asset is {} — larger than a vk build can be",
...
async fn install(client: &reqwest::Client, target: &Target, exe: &Path, dir: &Path) -> Result<()> {
    ...
    // `spawn::self_exe`): once the file this process was loaded from is unlinked, the
    // kernel reports it as `…/vk (deleted)`, and renaming onto that would install a
    // binary nobody runs while reporting success. Checked before the download, so a
    // pointless 40-megabyte transfer is not how the user finds out.
    if !exe.is_file() {
        bail!(
            "{} is gone — vk was replaced while this ran; rerun vk update",
            exe.display()
        );
    }
    ...
    // `vk` outright without going near this code.
    let tmp = dir.join(format!(".vk-update.{}", std::process::id()));
    ...
    let outcome = match download(client, target, file, &tmp, mode_of(exe)).await {
    ...
    OpenOptions::new()
        ...
        .map_err(|e| match e.kind() {
            std::io::ErrorKind::AlreadyExists => anyhow!(
                "{} already exists — an interrupted vk update left it behind, and nothing \
...
fn publish(tmp: &Path, exe: &Path, dir: &Path) -> Result<()> {
    ...
    // Best-effort, as in `vms::record_in`: the rename itself already succeeded, and a
    // host that cannot fsync its bin directory is no reason to report a failed update.
...
async fn download(
    ...
    // with the umask, so asking for 0755 there installs a 0700 `vk` under `umask 077`.
    ...
    // just after the rename could leave a truncated `vk` behind — and `vk` is the tool
...
async fn stream_asset(
    ...
    while let Some(chunk) = stream.next().await {
        ...
        // filesystem the installed `vk` lives on.
...
/// The `vk` asset's expected sha256, from the sidecar CI publishes beside it.
...
fn smoke_test(path: &Path, version: &str) -> Result<()> {
    ...
    // A whole token, not a substring: `--version` prints `vk-driver <version> (<hash>)`,
    ...
    if !out.status.success() || !named {
        bail!(
            "the downloaded vk did not report version {version} ({}, output: {})",
            out.status,
            reported.trim()
        );
    }
...
fn confirm() -> Result<bool> {
    if !std::io::stdin().is_terminal() {
        bail!("vk update needs a terminal to confirm at; pass --yes to update unattended");
    }
    eprint!("proceed? [y/N] ");
...
/// against `vk`'s own `CARGO_PKG_VERSION`.
...
/// another repository's releases, and both of this module's gates would then pass —
...
/// 64 hex digits as the 32 bytes they encode (as `ensure::parse_uuid` does for a UUID).
/// The length and alphabet are checked first, so the slicing below is in bounds and on
/// ASCII boundaries.
...
mod tests {
    ...
    /// A stand-in for the release's `vk`: a script, so the smoke test can really run it.
    ...
    /// Serve GitHub's release API and the two assets, for the one tag we publish, under
    /// `/<fault>/…` so a single server covers every case.
    fn serve(addr: SocketAddr, path: &str) -> Response<Full<Bytes>> {
        ...
        if rest == latest || rest == tagged {
            ...
            let json = format!(
                r#"{{"tag_name":"{FAKE_TAG}","assets":[
                     {{"name":"vk","browser_download_url":"http://{addr}/{seg}/vk","size":{size}}},
                     {{"name":"vk.sha256","browser_download_url":"http://{addr}/{seg}/vk.sha256","size":68}}
                   ]}}"#
            );
            return reply(200, json.into_bytes());
        }
        match rest {
            "vk" if fault == Fault::AssetError => reply(500, b"boom".to_vec()),
            "vk" => reply(200, body),
            "vk.sha256" if fault == Fault::SidecarError => reply(500, b"boom".to_vec()),
            "vk.sha256" if fault == Fault::HugeSidecar => {
                reply(200, format!("{FAKE_SUM}  vk\n").repeat(4096).into_bytes())
            }
            "vk.sha256" if fault == Fault::WrongVersion => {
                reply(200, format!("{WRONG_SUM}  vk\n").into_bytes())
            }
            "vk.sha256" => reply(200, format!("{FAKE_SUM}  vk\n").into_bytes()),
            _ => not_found(),
        }
    }

    /// The API root to hand `resolve` for a given fault, starting the one fake release
    /// server on first use. Its own thread + current-thread runtime, like `regproxy`'s
    /// fake upstream, so the accept loop never competes with a test's runtime threads.
                    ...
                    loop {
                        ...
                        tokio::spawn(async move {
                            ...
                            let svc = service_fn(move |req: Request<Incoming>| async move {
                                Ok::<_, Infallible>(serve(addr, req.uri().path()))
                            });
    ...
    /// The very client the subcommands use, so its configuration is on the tested path.
    fn test_client() -> reqwest::Client {
        // reqwest (rustls-no-provider) needs a crypto provider before a client builds;
        // `main` installs it for real runs.
        let _ = rustls::crypto::ring::default_provider().install_default();
        http_client().unwrap()
    }

    /// A scratch directory holding a stand-in for the installed `vk` at `mode`, removed
    /// however the test ends — a failed assertion must not leak it.
    ///
    /// Under `OUT_DIR` rather than `/tmp`: the smoke test execs the download from this
    /// directory, and a host that mounts `/tmp` `noexec` would fail every test here for
    ...
    impl Scratch {
        fn new(name: &str, mode: u32) -> Scratch {
            let dir = Path::new(env!("OUT_DIR")).join(format!("vk-update-{name}"));
    ...
    async fn install_publishes_a_download_that_passes_both_gates() {
        ...
        let target = resolve(&client, &release_api(Fault::None), None)
            .await
            .unwrap();
    ...
    // The point of the whole module: a download that fails either gate never becomes the
    ...
    async fn a_download_that_fails_a_gate_never_becomes_the_binary() {
        ...
        for (fault, want) in [
            ...
            let target = resolve(&client, &release_api(fault), None).await.unwrap();
            let err = format!(
                "{:#}",
    ...
    async fn a_rename_that_cannot_happen_leaves_no_temp_behind() {
        ...
        let target = resolve(&client, &release_api(Fault::None), None)
            .await
            .unwrap();
        let s = Scratch::new("rename", 0o755);
    ...
    async fn a_leftover_temp_is_reported_and_not_touched() {
        ...
        let target = resolve(&client, &release_api(Fault::None), None)
            .await
            .unwrap();
        let s = Scratch::new("leftover", 0o755);
