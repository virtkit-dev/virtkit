...
fn settle_index(dest: &Path) -> Result<()> {
    ...
    // A failed stat or backward clock step (Err from duration_since) skips only the wait.
    // The refresh still runs; it remains correct but may take the pre-settle slow path.
