...
//!
//! The terminal is left exactly as it was found. Raw mode is the caller's: it holds its own
//! guard, and hands [`catch_terminating_signals`] the settings [`current_termios`] read before
//! entering it. The alternate screen is held by a guard that restores on drop, so a panic or an
//! error path cannot leave a shell without its cursor — and because a signal is neither, the
//! terminating ones are caught long enough to restore it and then re-raised. What that handler
//! puts back is settled before it is installed and read from a plain static: a handler runs
//! between two instructions of whatever it interrupted, so everything it touches has to be safe
//! to touch there, locks included.
//!
...
/// to do when it cannot (a live attach records headless), rather than a flag to refuse.
/// [`crate::atop_view::view`] refuses on the same conditions; keep the two in step.
