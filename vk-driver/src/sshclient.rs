...
//! ProxyCommand. VS Code connects with bare `ssh` and copies its server with bare `scp`.
//!
...
/// The OpenSSH connection tools shimmed into [`SHIM_DIR`]. `sftp` takes the same `-F` as
/// `ssh` and `scp`, so it is shimmed too; each shim points the system tool at this run's
    ...
    fn shim_text(&self, tool: &str, path_var: Option<&OsStr>) -> Result<String> {
        ...
        Ok(format!(
            "#!/bin/sh\n\
             ...
             # A caller passing its own -F wins: the last one on the line takes effect.\n\
             exec '{real}' -F '{config}' \"$@\"\n",
            real = real.display(),
