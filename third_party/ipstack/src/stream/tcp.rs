...
async fn tcp_main_logic_loop(
    ...
    loop {
        ...
        if exhausted {
            // Retransmissions gave up on a segment the peer never acknowledged. Leaving the
            // connection Established would hand the application a stream with a permanent hole:
            // reads that never return and dup-ACKs we can no longer serve. Reset it instead so
            // the application fails fast and can reconnect.
            log::warn!("{network_tuple} {state:?}: retransmissions exhausted, resetting the connection");
