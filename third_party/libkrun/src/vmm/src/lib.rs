...
/// Process exit code meaning the guest asked to reset (reboot), as opposed to a
/// clean power-off (`FC_EXIT_CODE_OK`). Reported for a guest triple fault, a
/// `KVM_SYSTEM_EVENT_RESET`, the i8042 CPU-reset command, and the ACPI reset
/// register. A supervisor can relaunch the VM on this code to reboot it in place.
pub const KRUN_EXIT_GUEST_RESET: u8 = 154;
...
pub struct Vmm {
    ...
    /// Set by a device (i8042 CPU-reset, ACPI reset register) before it
    /// fires `exit_evt`, to mark the exit as a guest reset. See `KRUN_EXIT_GUEST_RESET`.
...
impl Subscriber for Vmm {
    ...
    fn process(&mut self, event: &EpollEvent, _: &mut EventManager) {
        ...
        if source == self.exit_evt.as_raw_fd() && event_set == EventSet::IN {
            ...
            // A reset takes precedence over every other code: whether it was
            // signalled by a device (reset_flag) or by a vcpu (triple fault /
            // KVM_SYSTEM_EVENT_RESET), the supervisor must see it to reboot.
            let exit_code = if self.reset_flag.load(Ordering::SeqCst)
                || vcpu_exit_code == KRUN_EXIT_GUEST_RESET
            {
                debug!("using guest-reset exit code");
                KRUN_EXIT_GUEST_RESET as i32
            } else if vmm_exit_code != i32::MAX {
