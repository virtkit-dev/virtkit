...
// libc dropped its musl `statx` struct/fn and STATX_* constants after 0.2.183
// (rust-lang/libc), but virtkit needs a newer libc. `struct statx` is defined by the
...
pub(crate) mod statx_compat {
    ...
    // Layout mirrors the kernel `struct statx` (256 bytes, arch-independent).
    #[repr(C)]
    #[derive(Clone, Copy)]
    pub struct Statx {
        pub stx_mask: u32,
...
fn statx(f: &File) -> io::Result<(libc::stat64, u64)> {
    let mut stx = MaybeUninit::<statx_compat::Statx>::zeroed();
    ...
    let res = unsafe {
        statx_compat::statx(
            ...
            pathname.as_ptr(),
            libc::AT_EMPTY_PATH | libc::AT_SYMLINK_NOFOLLOW,
            statx_compat::STATX_BASIC_STATS | statx_compat::STATX_MNT_ID,
            stx.as_mut_ptr(),
        )
    };
    if res >= 0 {
