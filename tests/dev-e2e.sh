#!/usr/bin/env bash
# =====================================================================================
# End-to-end gate for `vk dev`: one project, one config, every verb.
# =====================================================================================
...
# Needs: a `vk` with an embedded kernel/agent, KVM, network for the image pull, git, and
#        GNU-coreutils `timeout` — busybox's signals only its own child, so a hung guest
#        could outlive the step cap and leak a VM.
...
# GNU-coreutils timeout wraps every step and the cleanup stop; without it they are unguarded.
command -v timeout >/dev/null || { echo "SKIP: no timeout"; exit 0; }
...
cleanup() {
  ...
  for ws in "$WS" "$WS2"; do
    [ -d "$ws" ] && (cd "$ws" && timeout -k 15 120 "$VK" dev stop >/dev/null 2>&1) || true
  done
  rm -rf "$WORK"
}
...
PORT=$(free_port)
PORT2=$(free_port)
# Re-probe rather than settle for PORT+1, which free_port never checked was free.
while [ "$PORT" = "$PORT2" ]; do PORT2=$(free_port); done
...
# (its `nc` needs the `-lk`/`-e` extras, which Alpine's build has).
listener=$(cat <<'EOF'
mkdir -p /srv
cat > /srv/reply <<'INNER'
#!/bin/sh
