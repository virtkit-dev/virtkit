...
/// Whether an ancestor of `segs` is an object. Writing below one would make its path a
/// directory too, and [`resolve`] would then find the directory and hide the object: RFC 4918
/// wants a 409 for a parent that is not a collection. A top-level name is always a directory.
fn under_object(store: &Store, segs: &[String]) -> bool {
    (2..segs.len()).any(|n| matches!(resolve(store, &segs[..n]), Some(Node::Object(..))))
}
...
pub(super) async fn route(
    ...
    match method {
        ...
        "DELETE" => delete(store, segs, &this).await,
...
async fn put(
    ...
    // takes an object (405). Nor does a path below an object (409). Drain the body before
    // either answer.
    let (repo, tag) = match object_of(segs) {
        None => return drain_then(req, method_not_allowed(href, ALLOWED_FILES), href).await,
        Some(_) if store.repo_dir_exists(&repo_name(segs)) => {
            return drain_then(req, method_not_allowed(href, ALLOWED_FILES), href).await;
        }
        Some(_) if under_object(store, segs) => {
            return drain_then(req, parent_is_object(href), href).await;
        }
        Some(target) => target,
    };
...
/// Read a refused `PUT`'s body through, then answer `refusal`: a status sent with request
/// bytes unread resets the connection. 413 instead if the body crosses the cap on the way.
async fn drain_then(
    req: Request<Incoming>,
...
/// 409 for a write below an object: its parent is not a collection.
...
/// directory is already there, or 409 when an object is, there or above.
...
async fn delete(store: &Arc<Store>, segs: &[String], href: &str) -> Result<Response<Body>> {
    match resolve(store, segs) {
        ...
        Some(Node::Dir(repo)) => {
            // Off the runtime: the exclusive lock waits out every write in flight, and a gc.
            let store = Arc::clone(store);
            let removed = tokio::task::spawn_blocking(move || store.remove_empty_repo(&repo))
                .await
                .context("the directory's removal panicked")??;
            if removed {
                Ok(no_content())
            } else {
                Ok(error_response(
