...
use super::{
    ALLOWED_READ, Depth, Entry, blocking, href, method_not_allowed, multistatus, not_found,
    options, propfind_depth, refuse,
};
...
/// Serve one `repos/` request; `segs` are the decoded components after `/dav/repos/`, and
/// `collection` whether the path ended in a slash — which a tag, manifest or blob path never
/// does.
pub(super) async fn route(
    ...
    match method {
        "OPTIONS" => return Ok(options(ALLOWED_READ)),
        "GET" | "HEAD" | "PROPFIND" => {}
        _ => return refuse(req, method_not_allowed(&this, ALLOWED_READ), &this).await,
    }
    if method == "PROPFIND" {
        ...
        let enumerate = super::may_enumerate(state);
        let segs = segs.to_vec();
        return blocking(state, authz, move |store, authz| {
            match resolve(store, authz, &segs, collection, true) {
                Ok(node) => propfind(store, authz, &node, &segs, depth, enumerate),
                Err(resp) => Ok(*resp),
            }
        })
        .await;
    ...
    // No prefix walk for a download: a prefix has no body, and is a 404 either way.
    let node = match resolve(store, authz, segs, collection, false) {
        Ok(node) => node,
        Err(resp) => return Ok(*resp),
    };
    ...
    match &node {
        ...
        Node::Blob(name, hex) if crate::readable_through(authz, store, name, hex) => {
            let mut resp = crate::get_blob(store, &format!("sha256:{hex}"), head, accept_zstd)?;
            if resp.status() == StatusCode::OK {
                // Blob bytes are anyone's content; keep them from rendering on this origin.
...
/// accessing the repository on disk. `prefixes` resolves a component above readable
/// repositories, which walks every repository name; without it that is a 404.
fn resolve(
    store: &Store,
    authz: &Authz<'_>,
    segs: &[String],
...
/// `enumerate` is whether this server lists repositories at all (see `may_enumerate`).
fn propfind(
    store: &Store,
    authz: &Authz<'_>,
    node: &Node,
    ...
    let refused_enumeration = list && !enumerate;
    let enumerate = list && enumerate;
...
mod tests {
    ...
    fn paths_resolve_to_the_view_and_nowhere_else() {
        ...
        store
            ...
            .unwrap();
        let hex = "0".repeat(64);
        let r = |p: &str| resolve(&store, &authz, &segs(p), false, true);
        ...
        // a member's path with a trailing slash is a collection that is not there
        let as_collection = |p: &str| resolve(&store, &authz, &segs(p), true, true);
        assert!(matches!(
            as_collection("team-a/app/tags"),
            Ok(Node::Tags(_))
        ));
        assert_eq!(
            as_collection("team-a/app/tags/v1").err().unwrap().status(),
            404
        );
        // a download does not walk for prefixes
        assert_eq!(
            resolve(&store, &authz, &segs("team-a"), false, false)
                .err()
                .unwrap()
                .status(),
            404
    ...
    fn a_level_lists_the_next_component_of_each_readable_name() {
        ...
        assert!(matches!(
            resolve(&store, &authz, &segs("team-a"), false, true),
            Ok(Node::Repo(_))
        ));
        let _ = std::fs::remove_dir_all(&dir);
    }
