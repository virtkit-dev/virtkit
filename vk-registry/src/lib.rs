...
impl Store {
    ...
    /// anything is stored under it. Missing ancestors are created with it.
    ...
    pub(crate) fn remove_empty_repo(&self, name: &str) -> Result<bool> {
        ...
        // `remove_dir` refuses a directory with entries, so a tag written between the check
        // and the removal keeps the repository rather than being lost with it.
    ...
    /// Drop `name:tag`: the reference goes, the manifest and the blobs it names stay for
    /// [`Store::gc`] to sweep once idle. `Ok(false)` when there was no such tag.
    pub(crate) fn delete_tag(&self, name: &str, tag: &str) -> Result<bool> {
...
/// Collect garbage and print its summary.
