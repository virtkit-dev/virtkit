...
impl Store {
    ...
    /// `lstat`, so a symlink is neither followed nor shown.
    pub(crate) fn repo_children(&self, name: &str) -> Vec<String> {
        if !name.is_empty() && !valid_name(name) {
            return Vec::new();
        }
        let mut out: Vec<String> = std::fs::read_dir(self.root.join("repos").join(name))
            .ok()
            .into_iter()
            .flatten()
            .filter_map(|e| e.ok())
            ...
            .filter(|n| !REPO_SUBDIRS.contains(&n.as_str()) && valid_name(n))
            .collect();
        out.sort();
        out
    }
    ...
    /// Bring `name` into being empty: a `tags/` with nothing in it, which is what makes
    /// [`Store::has_repo`] answer — for a WebDAV `MKCOL`, where a directory exists before
    /// anything is stored under it. Missing ancestors are created with it. Under the shared
    /// lock, so a [`Store::remove_empty_repo`] of an ancestor cannot take it as it appears.
    ...
    /// Remove a repository that holds no tag and no nested repository. `Ok(false)` when it
    /// holds either. What else its directory may still hold — manifest sidecars and
    /// membership markers — records content no tag reaches any more, and goes with it.
    ///
    /// Under the exclusive lock, as [`Store::gc`] is: every write into a repository holds it
    /// shared, and a raw file records its membership markers before its tag, so without it a
    /// write between the check and the removal would lose its markers or its tag and still
    /// answer success. Blocks until those writes finish; call it off the async runtime.
    pub(crate) fn remove_empty_repo(&self, name: &str) -> Result<bool> {
        if !valid_name(name) {
            bail!("invalid repository name {name}");
        }
        let _lock = self.lock_exclusive()?;
        if !self.repo_children(name).is_empty() {
            return Ok(false);
        }
        let dir = self.root.join("repos").join(name);
        // A tag of its own keeps the repository: `remove_dir` refuses a `tags/` with entries.
        match std::fs::remove_dir(dir.join("tags")) {
            Ok(()) => {}
            Err(e) if e.kind() == std::io::ErrorKind::NotFound => {}
            Err(e) if e.kind() == std::io::ErrorKind::DirectoryNotEmpty => return Ok(false),
    ...
    /// Bump a tag's mtime — the "last used" record [`Store::gc`]'s retention keys on — for a
    /// use that did not go through [`Store::get_manifest`].
    ...
    /// Tie a stored blob to `name:tag` as a raw file: the shared empty config, the two
    /// membership records, and the single-layer manifest — the same writes a `/v2/` push
    /// makes. `size` is the blob's canonical length; `title` is kept as the OCI title
    /// annotation when there is one. Under the caller's shared lock, like every
    /// check→reference sequence. Returns the manifest digest.
    pub(crate) fn put_raw_file(
        ...
        // These bytes arrived through this server, for this repository, so they are readable
...
/// A raw-file manifest's config blob: fixed, empty content — a raw file has no build
/// config, but the OCI manifest schema requires a config descriptor. Every raw file
/// references the *same* config blob, which dedups after the first one.
...
mod tests {
    ...
    /// Removing an empty repository waits out a write in flight — held here as a WebDAV
    /// `PUT` holds it, across the membership markers and the tag — and then finds the
    /// repository no longer empty, rather than removing it from under that write.
    #[test]
    fn removing_an_empty_repo_waits_for_a_write_and_keeps_it() {
        ...
        let remover = {
            ...
            std::thread::spawn(move || tx.send(store.remove_empty_repo(name).unwrap()))
        };
        assert!(
            rx.recv_timeout(Duration::from_millis(200)).is_err(),
            "the removal waits while a write holds the lock"
        );
        ...
        assert!(!rx.recv().unwrap(), "the repository now holds a tag");
