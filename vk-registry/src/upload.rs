...
/// The blob, then the shared empty config and the single-layer manifest tying them together
/// ([`Store::put_raw_file`]) — the same three writes a `/v2/` push makes.
fn store_upload(
    store: &Store,
    name: &str,
    tag: &str,
    ...
    let title = file_name.map(clamp_name).filter(|t| !t.is_empty());
    store.put_raw_file(
        name,
        tag,
