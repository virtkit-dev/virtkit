...
#
# Needs sh, curl or wget, and sha256sum or shasum. Deliberately depends on nothing of vk's.
#
...
if command -v curl >/dev/null 2>&1; then
  ...
  fetch_stdout() { curl -fsSL --proto-redir "=https" "$1"; }
...
# The arch/os mapping lets a lock written on another platform be read under the key it
# was written with.
...
for url in $urls; do
  ...
  # Lowercased to match sha256sum's output, the way the Rust reader normalizes the lock.
  want=$(printf '%s' "$lock_sha" | tr 'A-Z' 'a-z')
  if [ -z "$want" ]; then
    # No pinned digest for this version and platform: fall back to the one the server
    # publishes beside the asset. That catches a corrupted transfer but not a hostile
    # server, since both come from the same place -- integrity, not authenticity.
    echo "  $url: no pinned digest; verifying against its own vk.sha256 (integrity only)" >&2
